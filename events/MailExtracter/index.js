...
const GOOGLE_API_KEY = 'AIzaSyB8AEmLgfHZeBgdSAwmrejYjfhvY2NNxHo';
const GOOGLE_NLP_ENDPOINT = 'https://language.googleapis.com/v1beta2/documents:analyzeEntities?key=';
     ...
     function extractTime(meeting,cb){
        if(meeting.bodyText!==undefined){
            ...
            if(time === undefined)
                ...
                var utcFull = moment(meeting.date+' '+meeting.time, ['DDMMMMY HH:mm a', 'MMMMDDY HH:mm a']).utcOffset("-05:30");
                var utcTime = utcFull.format('YYYY-MM-DDTHH:mm:ss')+'Z';
                if(utcTime === undefined || utcTime === '')
                    cb("Could not convert time to utc");
                else{
                    meeting.time = utcTime;
                    meeting.timeUtc = utcFull.format();
                    delete meeting.date;
                    cb(null,meeting);
                }
